...
int main(int argc, char *argv[]) {
  ...
  double local_prod;
   ...
   {
     int extra_local_N = local_N + extra;
     double local_x[extra_local_N];
     double local_y[extra_local_N];
     for(i = 0; i < local_N; i++) {
         local_x[i] = 0.01 * (i + my_rank * extra_local_N);
         local_y[i] = 0.03 * (i + my_rank * extra_local_N);
     }
